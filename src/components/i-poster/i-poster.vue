...
<script>
/**
	 * 插件市场链接： https://ext.dcloud.net.cn/plugin?id=5626
	 * github地址： https://github.com/similing4/xinyu-canvas-drawer-uniapp
...
export default {
  ...
  methods: {
    ...
    calcTextLines(text, size, maxWidth, isWrap) {
      ...
      var ret = [];
      if (!isWrap) maxWidth -= this.context.measureText('...').width;
      while (charArr.length != 0) {
        ...
        if (!isWrap) {
          if (charArr.length != 0) return ret + '...';
          else return ret;
        }
      }
      return ret;
     ...
     */
    getImageCache(src) {
      for (var i in this.imageCachePool) if (this.imageCachePool[i].key == src) return this.imageCachePool[i];
      return null;
    },
    /**
     * getImageInfo方法
    ...
    async draw() {
          ...
          if (item.type == 'image')
            return new Promise((recv, recj) => {
              ...
              if (item.data.image.startsWith('data:')) {
                ...
                env = swan.env;
                // #endif
                var filePath = env.USER_DATA_PATH + '/CANVASTEMP_' + sid + '.png';
                var buffer = uni.base64ToArrayBuffer(item.data.image.substring(item.data.image.indexOf(',') + 1));
            ...
            return new Promise(async (recv, recj) => {
              ...
              for (var i in item.data.extraConfig) config[i] = item.data.extraConfig[i];
      ...
      list.map(item => {
        if (item.type == 'image') {
          ...
          } else this.context.drawImage(item.data.image, item.data.x, item.data.y, item.data.w, item.data.h);
        } else if (item.type == 'text') {
          this.context.setTextBaseline('top');
          this.context.setFontSize(item.data.size);
          this.context.setFillStyle(item.data.color);
          var textArr = this.calcTextLinesWithNewLine(item.data.text, item.data.size, item.data.maxWidth, item.data.isWrap);
          for (var line = 0; line < textArr.length; line++) this.context.fillText(textArr[line], item.data.x, item.data.y + line * item.data.lineHeight);
        ...
        } else if (item.type == 'custom') item.data(this.context);
      });
